 ...
 */
package se.flapsdown.micrometer;
...
public abstract class AbstractTimeWindowHistogram<T, U> implements Histogram {
    ...
    public final HistogramSnapshot takeSnapshot(long count, double total, double max) {
        ...
        // Create snapshot
        HistogramSnapshot histogramSnapshot = new HistogramSnapshot(count, total, max, values, counts, this::outputSummary);
    ...
    private CountAtBucket[] takeCountSnapshot() {
        ...
        for (int i = 0; i < counts.length; i++) {
            ...
            double count = countAtValue(v);
            counts[i] = new CountAtBucket(v, count);
        }
        return counts;
    }
